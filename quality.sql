...
--
-- Estrutura da tabela `auditorias_certificacao`
--
...
--
-- Extraindo dados da tabela `auditorias_certificacao`
--
...
(3, 'Manufacture of cardboard packings in the process of off-set.', 'Produção de embalagens em papel kraft e papel cartão', 1, 87, 'Cotia - SP SEDE', 'Auditoria Principal', '2012-12-06', '2012-12-06', 1, 2);
...
--
-- Extraindo dados da tabela `auditorias_conclusoes`
--
...
--
-- Estrutura da tabela `auditorias_contato`
--
...
--
-- Extraindo dados da tabela `auditorias_contato`
--
...
--
-- Extraindo dados da tabela `auditorias_eficacia`
--
...
--
-- Extraindo dados da tabela `auditorias_notas`
--

INSERT INTO `auditorias_notas` (`id`, `auditor`, `data`, `processo`, `pessoal`, `notas`, `auditoria_id`) VALUES
(2, 'Juliana Bueno Colpas', '2012-12-06', 'Sistema de Gestão: Compras, Recebimento, Vendas.', 'Antonio Carlos Ferreira - Coodenador da Qualidade', 'Toda a sistemática de rastreabilidade é gerida no sistema informatizado da gráfica Romiti . Este sistema permite\r\nidentificar para cada Ordem de Produção, os números de Notas fiscais de entrada e saídas. Todos os registros ficam\r\narmazenados por 05 anos no mínimo.\r\nEvidenciada a sistemática de tratamento de reclamações de cliente, visando a Portaria 297 do Inmetro. A sistemática\r\nde tratativa segue também os critérios da ISO9001, uma vez que a Romiti também possui esta certificação.\r\nVerificada a sistemática de aprovação do uso da logomarca junto ao Inmetro .\r\nA estrutura de cada processo se dá:\r\nPROCEDIMENTOS\r\nTreinamento\r\nO Encarregado de Pessoal disponibiliza a todos novos colaboradores orientação quanto aos conceitos e requisitos\r\ngerais da certificação CERFLOR e os procedimentos específicos aplicáveis ao seu trabalho envolvendo a cadeia de\r\ncustódia; através de treinamentos, palestras e outros meios de informação;\r\nAnualmente a sistemática de cadeia de custódia é reciclada à todos colaboradores.\r\nA Capacitação dos envolvidos é registrada no RQ.21.\r\nVendas\r\nA Gráfica Romiti assegura que somente produtos fabricados de materiais CERFLOR puros e /ou CERFLOR baseados\r\nem porcentagem são comercializados como certificados;\r\nEsta garantia é evidenciada através de controle envolvendo as informações de entradas dos pedidos e sua condição\r\nde uso de produtos certificados CERFLOR bem como a clara identificação CERFLOR no cadastro dos produtos e\r\nmateriais certificados;\r\nTanto para pedidos novos quanto para pedidos repetidos , é levado em consideração se o produto final (cartucho)\r\nserá comercializado com selo CERFLOR. Esta condição pode ter sido estabelecida pelos clientes em suas\r\nespecificações técnicas ou solicitadas durante as negociações comerciais, ou por estratégia de marketing da Gráfica\r\nRomiti;\r\nNestes casos é registrado no Check-list de Vendas e na tela de Cadastro do Produto se a certificação será\r\nCERFLOR;\r\nPara todos os produtos da Romiti em que forem definidos a utilizarão do selo CERFLOR, não poderão ser utilizados\r\nou misturados, em hipótese alguma, cartões que não tenham origem certificada CERFLOR.\r\nNa emissão das faturas de materiais certificados são claramente identificados:\r\n- O nome e o endereço do comprador;\r\n- O código do certificado CERFLOR da empresa;\r\n- A data na qual a fatura foi emitida;\r\n- A quantidade de todos os produtos vendidos em cada grupo de produtos ;\r\n- No caso de material CERFLOR a nota fiscal deverá conter o número do certificado CERFLOR da Romiti.\r\nNo caso de material CERFLOR a nota fiscal deve constar o número do pedido do cliente para permitir a\r\nrastreabilidade referente ao período de entrega e de controle;\r\nCompras\r\nNas Requisições de Material e Pedidos de Compra é identificado a necessidade da aquisição de cartão certificado\r\nCERFLOR ou não;\r\nO Assistente de Compras confere junto ao requisitante do material a necessidade ou não de certificação CERFLOR .\r\nNos Pedidos de Compras enviados para os fornecedores constam claramente descritas as necessidades:\r\nIndicar se o material deverá ser certificado CERFLOR;\r\nExigir que o fornecedor envie a identificação da categoria de certificação CERFLOR e o nº do certificado em todos os\r\npaletes e na nota fiscal.\r\nTodo cartão deve ser cadastrado no sistema com a informação CERFLOR, desta forma o sistema informatizado\r\npoderá sempre identificar e rastrear os cartões que possuem certificação CERFLOR\r\nPara garantir esta identificação, em nenhuma hipótese estes tipos de cartões previamente definidos são comprados\r\nsem a certificação CERFLOR.\r\nNota: O Departamento da Qualidade possui uma cópia atualizada dos certificados CERFLOR dos fornecedores,\r\nindividualmente para cada fábrica certificada.\r\nRecebimento\r\nCabe ao Auxiliar do recebimento verificar se os materiais recebidos contem uma identificação e informações de\r\ncertificação conforme descrito no Pedido de Compra e requisitos pertinentes a fim de garantir a cadeia de custódia do\r\nselo CERFLOR;\r\nAlém dos critérios já definidos na ICG – Inspeção das Condições Gerais, também deverão ser inspecionados:\r\n- Rótulo em todos os Paletes: Identificação clara no material CERFLOR;\r\n- Nota Fiscal: deve conter declaração que o material é certificado CERFLOR e o número do certificado;\r\n- As informações descritas na Nota Fiscal devem ser consistentes com as etiquetas de todos os paletes;\r\n- Conferir o nº do certificado com a cópia do certificado na TB.02.\r\nCaso o item recebido não contenha o selo CERFLOR ou não puder ser identificado como CERFLOR, o Auxiliar do\r\nrecebimento rejeita o recebimento do item;\r\nEm nenhuma hipótese é aprovado cartão que não atenda aos critérios descritos anteriormente;\r\nA identificação do cartão certificado CERFLOR é a própria etiqueta do fornecedor , devidamente ins-pecionadas na\r\nentrada no material, não sendo necessária segregação física de material certificado. Apesar disto, é evitado que em\r\numa mesma fileira sejam armazenados materiais certificados com não certificados;\r\nCaso ocorra o envio de parte do material de um palete e haja saldo deste material certificado CERFLOR em estoque,\r\neste saldo é envolto em filme stretch para evitar qualquer mistura e é identificado com uma cópia da mesma etiqueta\r\ndo fornecedor, ou com a Ficha de Pilha interna da Romiti com a identificação CERFLOR. Esta mesma identificação\r\ndeverá ser aplicada para materiais que retornem do processo ou para pilhas que tenha sido perdida acidentalmente a\r\netiqueta do fornecedor;', 2);

-- --------------------------------------------------------

--
-- Estrutura da tabela `auditorias_observacoes`
--
...
--
-- Extraindo dados da tabela `auditorias_observacoes`
--
...
--
-- Estrutura da tabela `auditorias_revisao`
--
...
--
-- Extraindo dados da tabela `auditorias_revisao`
--

INSERT INTO `auditorias_revisao` (`id`, `numero`, `item`, `verificado`, `comentarios`, `auditoria_id`) VALUES
(1, '4.00', 'Requisitos para o X processo de cadeia de custódia - Método da separação física', 'Sim', NULL, 2);

-- --------------------------------------------------------

--
-- Estrutura da tabela `auditorias_sumarionc`
--
...
--
-- Extraindo dados da tabela `auditorias_sumarionc`
--
...
(4, 'ATACADÃO BAHIA', 'R. Barão de Mauá, 51C', 'BA', '2910800', '(75) 2101-0080', NULL, NULL, NULL),
...
--
-- Indexes for table `auditorias_eficacia`
--
...
ALTER TABLE `auditorias_conclusoes`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
-- AUTO_INCREMENT for table `auditorias_contato`
--
ALTER TABLE `auditorias_contato`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
...
ALTER TABLE `auditorias_revisao`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
-- AUTO_INCREMENT for table `auditorias_sumarionc`
--
ALTER TABLE `auditorias_sumarionc`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
