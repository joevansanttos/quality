...
<div class="col-md-3 col-sm-3 col-xs-12">
  ...
  <section class="panel">
    ...
    <div class="panel-body">
      <div class="row">
        <h3 class="green col-md-6 ">{{$p->cod}}</h3>
        <a  href="imprimir/{{$p->id}}"><button data-toggle="tooltip" data-placement="top" title="Imprimir Manual" class="pull-right btn btn-success"><i class=" fa fa-print "></i></button></a>
      </div>
      ...
      <div class="project_detail">
        ...
        <p class="title">Elaborado por</p>
        @if($p->usuario != null)
        <p>{{$p->usuario->name}}</p>
        @else
        <p></p>
        @endif
        <p class="title">Aprovado por</p>
        <p>{{$p->gestor}}</p>
        <p class="title">Unidade</p>
        <p>{{$p->unidade}}</p>
        <p class="title">Área</p>
        <p>{{$p->area}}</p>
        <p class="title">Data de Revisão</p>
        <p>{{$p->data_revisado}}</p>
      </div>
