...
class MicroController extends Controller
{
   ...
   public function microprocesso($id){
     ...
     return view ('pi_microprocesso_formulario')->with('s', $subprocesso)->with('classificacoes', Classificacao::all())->with('periodicidade', Periodicidade::all())->with('maturidade', Maturidade::all());;
   }
   ...
   public function mostra($id){
    ...
    $g = $m->gestor;
    $ks = $m->stakeholders;
    return view ('pi_microprocesso_detalhes')->with('m', $m)->with('c', Classificacao::find($m->classificacao_id))->with('p', Periodicidade::find($m->periodicidade_id))->with('t', Maturidade::find($m->maturidade_id))->with('g',($g))->with('ks',($ks));
  }

  public function stakeholder($id){
    $m = Microprocesso::find($id);
    return view ('pi_stakeholder_micro_formulario')->with('m', $m);
  }
  ...
  public function encontrar($id){
    $m = Microprocesso::find($id);
    $g = $m->gestor;
    return view ('pi_micro_editar_formulario')->with('m', $m)->with('classificacoes', Classificacao::all())->with('periodicidade', Periodicidade::all())->with('maturidade', Maturidade::all())->with('g',($g));
  }
